...
public class AbilityBlow : PawnAbility
{
...
// Member Functions	
	...
	protected void GetActionForce(GameObject _Go, ref float _fForce, ref Vector3 _vForceDirection)
	{
		...
		float fScaleCap = 0.2f;
		if(fDistance < m_fMaxDistance * fScaleCap)
		{
			fDistance = m_fMaxDistance * fScaleCap;
		}
		
		_fForce = m_fAccumulatedForce * (1.0f - (fDistance / m_fMaxDistance));
		
		if(_Go.GetComponent<Pawn>())
		{	
			_vForceDirection = _Go.transform.position - transform.position;
